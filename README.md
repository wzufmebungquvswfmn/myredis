...
这是我用 Rust 做的一个 Redis 风格小项目，主要想练习后端里比较核心的几件事：网络编程、协议解析、并发处理和工程化测试。

服务默认监听在 `127.0.0.1:6380`，可以直接用 `redis-cli` 连上去体验，也可以用 `redis-benchmark` 做压测。

## 1. 项目简介

这个项目我主要围绕三件事来做：先把功能做对，再保证并发场景可用，最后把测试和压测流程补完整。
