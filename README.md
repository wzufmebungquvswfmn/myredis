...
一个用 Rust + Tokio 实现的 Redis 风格内存数据库服务，目标是以清晰可读的工程结构实现 RESP 协议、常用数据结构命令与并发访问能力。

当前默认监听地址为 `127.0.0.1:6380`，可直接使用 `redis-cli` 或 `redis-benchmark` 进行验证。

## 1. 项目简介

myredis 目前聚焦于「协议正确性 + 并发可用性 + 可测性」三件事：
